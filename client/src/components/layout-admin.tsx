...
import { LayoutDashboard, Users, Dumbbell, CreditCard, LogOut, Menu, X } from "lucide-react";
      ...
      {navigation.map((item) => {
        ...
        return (
          <Link key={item.name} href={item.href}>
            <div onClick={onClick} className={`group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 cursor-pointer
              ${isActive ? "bg-primary text-primary-foreground shadow-lg shadow-primary/20" : "text-muted-foreground hover:bg-secondary hover:text-white"}`}>
  ...
  return (
    <div className="min-h-screen bg-background flex">
          ...
          <div className="p-4 mt-auto border-t border-border/50">
            <div className="flex items-center mb-4 px-2">
              <div className="h-9 w-9 rounded-full bg-primary/20 border border-primary/30 flex items-center justify-center">
                <span className="text-primary font-bold text-sm">{user?.firstName?.[0]?.toUpperCase()}</span>
              </div>
              <div className="ml-3">
            ...
            <Button variant="outline" className="w-full justify-start text-muted-foreground hover:text-white hover:bg-secondary border-border/50" onClick={() => logout()}>
        ...
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
          ...
          <SheetContent side="left" className="w-64 bg-card border-r border-border p-0">
            <div className="flex flex-col h-full pt-5">
              ...
              <div className="flex-1 px-4 space-y-1"><NavLinks onClick={() => setIsOpen(false)} /></div>
      ...
      <div className="flex-1 flex flex-col md:pl-64 pt-16 md:pt-0">
        <main className="flex-1 py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">{children}</main>
      </div>
    </div>
  );
}
